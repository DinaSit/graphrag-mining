...
Проект сделан как Python/FastAPI backend с заменяемыми портами для парсеров, LLM, embeddings, vector store и graph store. Извлечение знаний вынесено в отдельный HTTP-сервис `ml-mock`, поэтому mock-реализацию можно заменить реальной моделью без переписывания API и UI.
...
10. Source fragments индексируются deterministic embedding provider-ом в pgvector.
...
backend/
  app/
    ...
    pipeline/
      ...
      providers.py          MockLLM, RemoteExtractionProvider, deterministic embeddings
      normalization.py      ontology/synonyms/unit normalization
      validation.py         regex + Pint-compatible numeric validation
...
HTTP adapter к внешнему сервису извлечения. Отправляет fragments на `EXTRACTION_SERVICE_URL` и ожидает JSON с candidates. Если сервис недоступен или вернул некорректный JSON, используется fallback на `MockLLMProvider`.

Это позволяет запускать систему автономно и при необходимости подключить реальный extraction service без изменения REST API и UI.
...
- нормализует температуру, длительность, проценты, концентрации, скорость потока;
...
### Fail-soft external adapters

`RemoteExtractionProvider` падает обратно в mock extraction, если внешний сервис извлечения недоступен. `MinioFileStorage` не валит весь ingest при временной ошибке MinIO. Это повышает удобство локальной разработки.
...
Core tests находятся в `backend/tests/test_core.py`.

Проверяется:

- sample data создает вертикальный срез документов, фактов и vectors;
- контрольный вопрос возвращает эксперименты, источники и graph payload;
- question parser извлекает материал и температурный диапазон;
- факт нельзя утвердить без источника;
- ingest записывает `storage_uri`;
- XLSX parser извлекает строки из workbook.
...
Текущая версия уже поднимает полный локальный контур и показывает сквозной сценарий. Для перехода от mock-first режима к более сильной версии нужно усилить несколько частей:

- заменить mock extraction на реальный LLM extraction service;
- подключить embedding model уровня `bge-m3`;
...
- Extraction и embeddings работают в mock-first режиме.
...
- Таблица `jobs` есть, но ingest пока синхронный.
