#!/usr/bin/env python3
"""Диагностика доступа к Yandex AI Studio.

Проверяет: доступность чата и его латентность, соблюдение JSON-формата,
приём изображений (vision), эмбеддинги doc/query и их размерность,
...
async def main() -> int:
    ...
    # --- 4. Эмбеддинги doc/query ---
    for kind in ("doc", "query"):
        try:
            t0 = time.monotonic()
            vecs = await yandex_client.embed(["обессоливание шахтных вод", "electrowinning"], kind=kind)
            print(f"[4] embeddings {kind}: OK за {time.monotonic() - t0:.1f}с, размерность {len(vecs[0])}")
        except Exception as e:
            ok = False
            print(f"[4] embeddings {kind}: FAIL — {e}")
