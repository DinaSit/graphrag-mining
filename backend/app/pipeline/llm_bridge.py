...
# Единственное место, где отказ модели превращается в текст для человека.
# Технические подробности (тело ответа провайдера, код HTTP) остаются в логах:
# в интерфейс они выносят внутренности провайдера — вплоть до фрагмента ключа
# и имени учётной записи — и обрываются на полуслове при усечении
_KIND_HUMAN = {
    "auth": "Модель недоступна: ключ отклонён",
    "quota": "Лимит модели исчерпан",
    "bad_response": "Модель вернула некорректный ответ",
