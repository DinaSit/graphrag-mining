...
class QueryOrchestrator:
    ...
    async def answer(self, request: QueryRequest) -> QueryResponse:
        ...
        # Семантический поиск не зависит от LLM: считается всегда и возвращается
        # пользователю (раньше жил только внутри evidence pack)
        try:
            search_hits = self.store.search(request.question, top_k=10)
        except Exception:
            search_hits = []
    ...
    def _find_contradictions(self, facts: list[Fact]) -> list[str]:
        ...
        for fact in facts:
            ...
            groups.setdefault(key, set()).add(_direction_key(fact.effect_direction))
        ...
        for key, directions in groups.items():
            if "increase" in directions and "decrease" in directions:
                ...
                contradictions.append(
                    ...
                    f"лаборатории: {', '.join(sorted(labs[key]))}."
                )
    ...
    def _find_gaps(self, parsed, facts: list[Fact], numeric_evidence: list[dict[str, Any]]) -> list[str]:
        ...
        if parsed is not None and parsed.target and not numeric_evidence and not facts:
